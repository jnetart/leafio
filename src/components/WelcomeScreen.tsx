...
export function WelcomeScreen({
  ...
  return (
    <div className="welcome-screen flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-[var(--window-bg)]">
      <div className="scroll-pane flex min-h-0 flex-1 flex-col overflow-y-auto">
        <div className="flex min-h-full w-full flex-col items-center justify-center px-8 py-12">
          ...
          <div className="welcome-screen__actions mt-8 flex w-full max-w-[360px] flex-col items-stretch gap-3">
            <button
              type="button"
          ...
          {showWorkspaceList && workspaceRoots.length > 0 ? (
            <div className="mt-8 w-full max-w-[360px]">
              <div className="mb-2 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">
                {labels.workspace}
              </div>
          ...
          {recentFiles.length > 0 ? (
            <div
              className={`w-full max-w-[360px] ${
                showWorkspaceList && workspaceRoots.length > 0 ? 'mt-6' : 'mt-8'
              }`}
            >
              <div className="mb-2 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">
                {labels.recent}
              ...
              <div className="space-y-1">
                {recentFiles.map((path) => (
                  <button
                    ...
                    title={path}
                    className="flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-[12px] hover:bg-black/5 dark:hover:bg-white/[0.05]"
                  >
                    <IconFile className="h-3.5 w-3.5 shrink-0 opacity-50" />
                    <span className="truncate">{formatDisplayPath(path, homeDir)}</span>
                  </button>
                ))}
              </div>
            </div>
