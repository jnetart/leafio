...
describe('filterBlockActions', () => {
  it('returns all actions for an empty query', () => {
    expect(filterBlockActions('')).toHaveLength(15);
  });
  ...
  it('matches titles and keywords', () => {
    ...
    expect(filterBlockActions('脚注').map((item) => item.id)).toEqual(['footnote']);
    expect(filterBlockActions('footnote').map((item) => item.id)).toEqual(['footnote']);
  });
